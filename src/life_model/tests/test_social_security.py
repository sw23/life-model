...
class TestSocialSecurity(unittest.TestCase):
    def test_get_aime_caseA(self):
        ...
        person = Person(family=Family(model), name="Test A", age=22, retirement_age=62, spending=Spending(model, 10000))
        ...
        for (
            chk,
            val,
        ) in zip(chk_scaled_income, indexed_income_history):
    ...
    def test_get_aime_caseB(self):
        model = LifeModel(start_year=1983)
        person = Person(family=Family(model), name="Test B", age=26, retirement_age=62, spending=Spending(model, 10000))
    ...
    def test_get_early_delayed_pia(self):
        model = LifeModel(start_year=1983)
        person = Person(family=Family(model), name="Test C", age=26, retirement_age=62, spending=Spending(model, 10000))
